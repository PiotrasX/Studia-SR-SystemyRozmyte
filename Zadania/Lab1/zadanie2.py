...
quality_value_data = np.arange(0, 10.01, 0.01)
tip_out = []

# Zbieranie wyników dla każdej wartości ostrej
for qv in quality_value_data:
    tipper_simulation = ctrl.ControlSystemSimulation(tipper_ctrl) # Symulacja działania sterownika rozmytego
    tipper_simulation.input['Jakość obsługi'] = qv # Ustawienie wejścia ostrego
    tipper_simulation.compute() # Przeprowadzenie symulacji
    tip_out.append(tipper_simulation.output['Napiwek w %']) # Zapisanie wyniku symulacji

# Wizualizacja zależności napiwku od jakości obsługi
plt.figure()
plt.plot(quality_value_data, tip_out)
plt.xlim(0, 10)
plt.ylim(0, 20)
plt.xlabel('Jakość obsługi')
plt.ylabel('Napiwek w %')
plt.title('Wysokość napiwku na podstawie jakości obsługi')
plt.grid(True)
plt.show()
